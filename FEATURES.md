...
| **Calendar** | Publish accepted blocks, move future sessions to rest days, swap sessions, reconcile supported Intervals.icu moves | Inbound moves are conservative. Publication serialization is limited to one server process. [Concurrency contract](docs/systems/06-generation.md#the-two-phase-commit). |
